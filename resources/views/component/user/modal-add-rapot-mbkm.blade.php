    ...
    <div class="relative p-4 w-full max-w-md max-h-full bg-white rounded-lg shadow">
        ...
        <div class="flex items-center justify-between p-4 border-b">
            <h3 class="text-lg font-semibold text-gray-900">
                Tambahkan Projek Yang Di Ikuti Siswa
            </h3>
        ...
        <!-- Modal body -->
        <form id="mbkm-form" action="{{ route('store.raport.mbkm') }}" method="POST">
            @csrf
            ...
            <div class="grid gap-4 my-4">
                <div>
                    <label for="project" class="block text-sm font-medium text-gray-900">Judul Projek</label>
                    <select name="id_project" id="id_project"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                        required>
                        <option value="">--PILIH JUDUL--</option>
                        @foreach ($projects as $prject)
                            <option value="{{ $prject->id }}">{{ $prject->judul }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
