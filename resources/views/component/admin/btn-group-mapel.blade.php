<span class="inline-flex -space-x-px overflow-hidden rounded-md border bg-white shadow-sm">
    <a href="{{ route('edit.mapel', $item->id) }}" class="inline-block px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 focus:relative">
        Edit
    </a>

    <form action="{{ route('destroy.mapel', $item->id) }}" method="POST" class="inline-block">
        @csrf
        @method('DELETE')
